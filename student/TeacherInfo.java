...
class TeacherInfo extends JPanel implements ActionListener,Listener {// 学生信息管理
	...
	public void getAllStudents(){
		...
		try {
			rs = stmt.executeQuery("select * from t order by tno");
			while (rs.next()) {
	...
	public void delete() {
		...
		if (row == -1) {// 判断要删除的信息是否被选中
			...
			try {
				stmt.executeUpdate("delete from c where tname='"+tname+"'");   //删除选课表中的记录
				stmt.executeUpdate("delete from t where tno='"+tno+"'");    //删除学生表中的记录
	...
	public void getMessage(String message) {
		...
		connDB();
		try {
			rs = stmt.executeQuery("select * from t where tno='" +message+ "'");
			rs.next();
