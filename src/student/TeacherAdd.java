...
class TeacherAdd extends JFrame implements ActionListener{
	...
	JLabel lsno = new JLabel("院工号：");
	JLabel lsname = new JLabel("姓名：");
	JLabel lssex = new JLabel("用户名：");
	...
	JTextField tssex = new JTextField(14);
	...
	public TeacherAdd(Listener listener) {// 构造方法
		...
		tssex.setBounds(110, 100, 120, 20);
		getContentPane().add(tssex);
	...
	public void update(){
		...
		try{
			...
			if(xm.equals("")){
				...
				String sql="update t set 院负责人姓名='"+xm+"',用户名='"+xb+"' where 院工号='"+xh+"'";
				try {
	...
	public void insert() { // 插入记录
		...
		try 
		{
			...
			{
				JOptionPane.showMessageDialog(null, "学号、姓名、不能有空值！");
			}
			else
			{
